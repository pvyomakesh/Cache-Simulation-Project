...
void setSizesOffsetsAndMaskFields(cache* acache, unsigned int size, unsigned int
assoc, unsigned int blocksize){
...
acache->VATmask=(uint64_t)(((uint64_t)1)<<(localVAbits - acache->TO)) -
(uint64_t)1; // computes AND mask for tag
}
...
void writeback(cache* acache, unsigned int index, unsigned int oldestway){
    uint64_t baseaddress = acache->sets[index].blocks[oldestway].tag << (acache->TO) | (index << acache->BO); 
    for (unsigned int i = 0; i < acache->blocksize/8; ++i) {
        performaccess(acache->nextcache, baseaddress, 8, 1, acache->sets[index].blocks[oldestway].datawords[i], i);
        baseaddress += 8; 
}
}
void fill(cache * acache, unsigned int index, unsigned int oldestway, unsigned long long address){
    uint64_t baseaddress = address >> acache->BO;
    baseaddress = baseaddress << acache->BO; 
    for (unsigned int i = 0; i < acache->blocksize/8; ++i){
        acache->sets[index].blocks[oldestway].datawords[i] = performaccess(acache->nextcache, baseaddress, 8, 0, 0, i);
        baseaddress += 8;
